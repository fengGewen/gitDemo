...
- (void)leftHeadAnimation {
    ...
    for (int index = 1; index < self.dataKey.count; index++) {
        ...
        for (int i = 0; i<index; i++) {
            NSArray *array = [self.dataSource objectForKey:self.dataKey[i]];
            count += array.count;
        }
        CGFloat headTopY = index*LCHeadHeight + count*LCRowHeight;
        CGFloat headTopHeight = headTopY + LCHeadHeight;
